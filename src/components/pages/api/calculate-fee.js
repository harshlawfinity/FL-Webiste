 export default function handler(req, res) {
  ...
  const { state, form } = req.body;
  console.log("👉 Received request:", { state, form });
  ...
  const stateConfigs = {
    UP: {
      ...
      ranges: {
        CTE: {
          slabs: [9, 50, 100, 1000, 5000, 10000, 50000],
          fees: [500, 1500, 3000, 10000, 25000, 37000, 50000],
        },
        CTO: {
          slabs: [10, 50, 100, 1000, 5000, 10000, 50000],
          fees: [1000, 3000, 8000, 40000, 100000, 150000, 200000],
        },
      },
      calculateFee: (form) => {
  ...
  try {
    const feeResult = config.calculateFee(form);
    
    return res.status(200).json({ ...feeResult });
  } catch (e) {
    return res.status(500).json({ message: "Error in calculation", error: e.message });
  }
}
